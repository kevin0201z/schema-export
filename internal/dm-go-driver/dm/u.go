...
func (st *DmStatement) prepare() error {
	...
	st.execInfo, err = st.dmConn.Access.Dm_build_493(st, Dm_build_788)
	if err != nil {
		return err
	}
...
func (stmt *DmStatement) exec(args []driver.Value) (*DmResult, error) {
	...
	if stmt.isBatch && len(args) > 0 {
		...
		err = stmt.executeInner(args, Dm_build_788)
	}
	if err != nil {
		return nil, err
	}
...
func (stmt *DmStatement) query(args []driver.Value) (*DmRows, error) {
	var err error
	stmt.inUse = true
	err = stmt.executeInner(args, Dm_build_789)
	if err != nil {
		return nil, err
	}
...
func checkNull(arg interface{}) bool {
	if arg == nil {
		return true
	}

	innerValuer, ok := arg.(driver.Valuer)
	if ok {
		innerValue, err := innerValuer.Value()
		if err == nil && innerValue == nil {
			return true
		}
	}
	return false
...
func (stmt *DmStatement) executeBatch(args []driver.Value) (err error) {
	...
	if stmt.execInfo.retSqlType == Dm_build_803 || stmt.execInfo.retSqlType == Dm_build_808 {
	...
	if stmt.paramCount > 0 && args != nil && len(args) > 0 {
		...
		if len(args) == 1 || stmt.dmConn.dmConnector.batchType == 2 ||
			(stmt.dmConn.dmConnector.batchNotOnCall && stmt.execInfo.retSqlType == Dm_build_804) {
